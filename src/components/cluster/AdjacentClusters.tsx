...
export default function AdjacentClusters({ adjacent }: { adjacent: Cluster[] }) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-display font-bold text-ink-muted">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {adjacent.map((adj) => (
          <Panel key={adj.id} href={`/cluster/${adj.id}`} accent="coral" className="p-5">
            <div className="flex items-center justify-between font-mono text-[8px] text-ink-muted uppercase mb-2">
              <span>{adj.categoryLabel}</span>
              <SignalMeter value={adj.memberCount} max={maxSignal} size="sm" label={`${adj.memberCount}`} />
            </div>
            <p className="text-ink-muted text-xs italic font-medium line-clamp-2">
              &quot;{adj.canonicalText}&quot;
            </p>
          </Panel>
        ))}
